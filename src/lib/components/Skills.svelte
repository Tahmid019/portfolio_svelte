<script lang="ts">
	...
	const tabs: string[] = ['Tools', 'Interests', 'Statistics'];
...
<section id="skills" class="">
	...
	<BlurFade>
		...
		<div use:reveal class="mx-5 sm:hidden">
			...
			<select
				id="tabs"
				class="block w-full transform rounded-lg border-2 border-gray-300 bg-[#000] p-2.5 text-sm text-[#858585] transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-[#9b79f71c] focus:scale-105 focus:border-gray-200 focus:shadow-lg focus:shadow-[#7986f71c] focus:ring-2 focus:ring-gray-200 focus:ring-opacity-50 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400"
			...
			<div class="radio-container m-5 ml-0 hidden h-min flex-row justify-center sm:flex">
				{#each tabs as tab, index}
					<div class="relative">
						...
						<label
							for={`radio-${index}`}
							class="relative cursor-pointer p-4 text-gray-500 transition-all duration-300 hover:text-gray-200 {activeTab ===
							index
